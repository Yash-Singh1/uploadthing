...
export const INTERNAL_uploadthingHookGenBuilder =
  (pkgName: string) =>
  <TRouter extends FileRouter>(initOpts: {
    /**
    ...
    const useUploadThing = <TEndpoint extends keyof TRouter>(
      ...
      const startUpload = useEvent(async (...args: FuncInput) => {
        ...
        try {
          const res = await DANGEROUS__uploadFiles<TRouter, TEndpoint>(
            endpoint,
            {
              files,
              input,
...
export const generateReactHelpersBuilder =
  (pkgName: string) =>
  <TRouter extends FileRouter>(initOpts?: GenerateTypedHelpersOptions) => {
    const url = resolveMaybeUrlArg(initOpts?.url);

    return {
      useUploadThing: INTERNAL_uploadthingHookGenBuilder(pkgName)<TRouter>({
        url,
      }),
